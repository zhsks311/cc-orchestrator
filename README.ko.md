...
- `npx cc-orchestrator@latest --upgrade` 는 기존 설치를 최신 published release tag로 이동시킵니다.
- 기존 clone 안에서 `npm run update` 를 실행하면 최신 release tag를 다시 checkout한 뒤 `npm install` 과 `npm run setup -- --yes` 를 다시 실행합니다.
