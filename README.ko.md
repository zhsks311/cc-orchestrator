...
- `npx cc-orchestrator@latest` 는 `main`이 아니라, 해당 npm installer 버전에 대응하는 release git tag를 설치합니다.
- `npx cc-orchestrator@latest --upgrade` 는 최신 npm-published installer version에 대응하는 release tag로 이동시킵니다.
- `npm run update` 는 npm registry에서 최신 installer version을 확인한 뒤, 그에 대응하는 release tag를 checkout하고 `npm install` 과 `npm run setup -- --yes` 를 다시 실행합니다.
